...
do while( time .le. time_max )
 ...
 ! Forces at the boundaries
  if( io_forc==1 .and. mod(nloop, 1000)==0) then
      ...
      write (1,'(i10,1x,f7.3,1x,e10.3,1x,e10.3,1x,e10.3,1x,e10.3,1x,e10.3,1x,i10,1x,e10.3)') nloop,time/sec_year/1.d6, &
      force_l, force_r,ringforce, vl,vr, nloop-lstime, limit_force
      close (1)
      ...
      if ( itype_force==1.and.abs(nloop-lstime).gt.1000000) then 
          lstime = nloop
          !$ACC parallel loop async(1) 
          do j=1,nz
